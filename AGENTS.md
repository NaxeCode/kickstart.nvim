...
- **Swift / Apple platforms:** Mac-only SourceKit-LSP from the selected Xcode toolchain, Swift TreeSitter, SwiftFormat, SwiftLint, and Aura Gainz `apple-verify` Overseer workflows under `<leader>a`. Do not install SourceKit through Mason or mutate generated Xcode projects. See `SWIFT_DEVELOPMENT.md`.
